// api/football-daily.js
// Cron giornaliero 07:00 UTC — esegue tutto in sequenza
// Fixtures → xG → Elo → Odds → Algo
// Usa puppeteer-core + @sparticuz/chromium per Vercel serverless
...
const TEAM_NAME_MAP = {
  // Premier League — football-data.org → Understat
...
const ODDS_NAME_MAP = {
  // Bundesliga — The Odds API → Understat
...
function normFD(name)   { return TEAM_NAME_MAP[name]   || name; }
  ...
  for (const league of LEAGUES) {
    ...
    for (const m of matches) {
      const { error } = await supabase.from('football_games').upsert({
        match_id:   `fd_${m.id}`,
        league:     league.code,
        home_team:  normFD(m.homeTeam.name),
        away_team:  normFD(m.awayTeam.name),
...
// ─── STEP 2: xG DA UNDERSTAT ─────────────────────────────────────────────────

async function fetchXG() {
  console.log('\n[2/5] xG UNDERSTAT');

  // Usa puppeteer-core + chromium per Vercel serverless
  const isVercel = !!process.env.VERCEL;
  const browser = await puppeteer.launch(isVercel ? {
    args: chromium.args,
    executablePath: await chromium.executablePath(),
    headless: chromium.headless,
  } : {
    headless: true,
    executablePath: puppeteer.executablePath ? undefined : undefined,
  ...
  try {
    for (const league of LEAGUES) {
      const url = `https://understat.com/league/${league.understat}/${SEASON}`;
  ...
  const scripts = document.querySelectorAll('script');
      ...
      if (!teamsData) { console.log(`  ❌ ${league.understat}: teamsData non trovato`); continue; }
...
async function fetchElo() {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 5) eloMap[parts[1]?.trim()] = parseInt(parts[4]?.trim());
    }
    ...
    console.log(`  ⚠️  ClubElo errore: ${e.message} — skip`);
  }
}
  ...
  for (const league of LEAGUES) {
    ...
    for (const ev of events) {
      ...
      const { data: games } = await supabase.from('football_games')
        .select('match_id').eq('league', league.code)
        .eq('home_team', homeN).eq('away_team', awayN);
      if (!games?.length) continue;

      await supabase.from('football_games').update({
        ou_quote_pin: pinOver || null, ou_quote_book: bookOver || null,
        ou_line_source: bookOver ? 'ODDS_API' : 'PINNACLE',
        updated_at: new Date().toISOString(),
...
async function runAlgo() {
  ...
  const { data: games } = await supabase.from('football_games')
    .select('*').gte('match_date', now.toISOString())
    .lte('match_date', in48h.toISOString()).eq('status','TIMED');
  ...
  for (const g of games || []) {
    ...
    await supabase.from('football_games').update({
      ...
      xg_gap: gap, ou_line: line,
      signal, signal_strength: strength, btts_signal: btts,
      updated_at: new Date().toISOString(),
...
export default async function handler(req, res) {
  console.log(`\n${'='.repeat(50)}`);
  console.log(`FOOTBALL DAILY — ${new Date().toISOString()}`);
